...
namespace Infrastructure.WorkTime
{
    public class HeadPoseNormalization
    {
        ...
        public Mat NormalizePosition(Mat src, Rect faceRect, List<IDictionary<FacePart, IEnumerable<Point>>> landmarks = null)
        {
            if (landmarks == null)
            {
            ...
            var bml = landmarks.First()[FacePart.BottomLip].First(v =>
                v.X == landmarks.First()[FacePart.BottomLip].ElementAt(0).X);
            innerEyesAndBottomLip[2] = points
                .Select((p, i) => new { p, i })
                .Where((v, i) => v.p.X == bml.X && v.p.Y == bml.Y)
                .Select(v => v.i).First();


            var el = landmarks.First()[FacePart.LeftEye]
                .First(v => v.X == landmarks.First()[FacePart.LeftEye].Min(p => p.X));
            innerEyesAndBottomLip[0] = points
                .Select((p, i) => new { p, i })
                .Where((v, i) => v.p.X == el.X && v.p.Y == el.Y)
                .Select(v => v.i).First();

            var er = landmarks.First()[FacePart.RightEye]
                .First(v => v.X == landmarks.First()[FacePart.RightEye].Max(p => p.X));
            innerEyesAndBottomLip[1] = points
                .Select((p, i) => new { p, i })
                .Where((v, i) => v.p.X == er.X && v.p.Y == er.Y)
                .Select(v => v.i).First();
            ...
            var h = Cv2.GetAffineTransform(p1.Select(p =>
                    new Point2f((p.X),
                        (p.Y))),
                p2.Select(p => new Point2f(p.X * faceRect.Width, p.Y * faceRect.Height)));
